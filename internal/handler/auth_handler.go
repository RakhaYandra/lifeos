...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	if errors.Is(err, service.ErrEmailTaken) {
		...
		return
	} else if errors.Is(err, service.ErrSingleUser) {
		c.JSON(http.StatusForbidden, gin.H{"error": "single_user_only"})
		return
	} else if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "internal"})
		return
	}
	_ = h.Settings.EnsureDefaults(id, 2026)
	c.JSON(http.StatusCreated, gin.H{"id": id, "email": in.Email})
}
