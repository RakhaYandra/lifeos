...
func (f *fakeUsers) FindByEmail(email string) (*repository.UserRow, error) {
	...
	return &repository.UserRow{ID: 9, Email: email, PasswordHash: f.hash}, nil
}
func (f *fakeUsers) Create(email, hash string) (int64, error) { return 10, nil }
func (f *fakeUsers) Count() (int, error)                      { return f.count, nil }
...
func TestRegisterSingleUser(t *testing.T) {
	svc := &AuthService{Users: &fakeUsers{count: 1}, Secret: "s"}
	if _, err := svc.Register("baru@example.com", "Rahasia123"); !errors.Is(err, ErrSingleUser) {
		t.Fatalf("mau ErrSingleUser, dapat %v", err)
	}
}
